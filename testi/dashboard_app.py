...
def display_live_flows(db_path='sessions.db', limit=20):
    ...
    for _, row in df.head(limit).iterrows():
        ...
        df_data.append({
            ...
            'Anomaly': f"{float(row.get('anomaly_score')):.3f}" if pd.notna(row.get('anomaly_score')) else "N/A",
            'Endpoint': f"{float(row.get('endpoint_score')):.3f}" if pd.notna(row.get('endpoint_score')) else "N/A",
            'Threat Intel': f"{float(row.get('ti_score')):.3f}" if pd.notna(row.get('ti_score')) else "N/A",
            'IAM': f"{float(row.get('iam_score')):.3f}" if pd.notna(row.get('iam_score')) else "N/A",
            'Combined': f"{float(row.get('combined_score')):.3f}" if pd.notna(row.get('combined_score')) else "N/A",
...
def display_risk_gauge(db_path='sessions.db'):
    ...
    active_df = df[df['state'].fillna('').str.startswith('CLOSED') == False] if 'state' in df.columns else df
    use_df = active_df if not active_df.empty else df
...
def display_risk_breakdown(db_path='sessions.db'):
    ...
    risk_scores = [get_risk_score_from_db(row) for _, row in use_df.iterrows()]
    avg_risk = np.mean(risk_scores) if risk_scores else 0.5
    
    # Decision score distribution
    decision_scores = use_df['decision_score'].dropna().tolist() if 'decision_score' in use_df.columns else []
    avg_decision_score = np.mean(decision_scores) if decision_scores else avg_risk

    avg_anomaly = float(use_df['anomaly_score'].dropna().mean()) if 'anomaly_score' in use_df.columns and not use_df['anomaly_score'].dropna().empty else None
    avg_endpoint = float(use_df['endpoint_score'].dropna().mean()) if 'endpoint_score' in use_df.columns and not use_df['endpoint_score'].dropna().empty else None
    avg_ti = float(use_df['ti_score'].dropna().mean()) if 'ti_score' in use_df.columns and not use_df['ti_score'].dropna().empty else None
    avg_iam = float(use_df['iam_score'].dropna().mean()) if 'iam_score' in use_df.columns and not use_df['iam_score'].dropna().empty else None
    
    # Action distribution
    if 'decision_action' in use_df.columns:
        blocked_ratio = len(use_df[use_df['decision_action'].str.contains('BLOCK|DENY|QUARANTINE', case=False, na=False)]) / len(use_df) if len(use_df) > 0 else 0
    else:
        blocked_ratio = 0
    ...
    score_components = {
        'Anomaly': avg_anomaly,
        'Endpoint': avg_endpoint,
        'Threat Intel': avg_ti,
        'IAM': avg_iam,
        'Final Risk': avg_risk,
    }
    comp_df = pd.DataFrame([
        {'Component': k, 'Average': v if v is not None else np.nan}
        for k, v in score_components.items()
    ])
    st.markdown("#### Score Component Averages")
